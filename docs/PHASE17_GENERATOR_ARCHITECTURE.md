...
Phase 17-2 迁移定价、模型规范化和发布逻辑；Phase 17-3 继续迁移渲染、通知并将 `generate.py` 收缩为稳定 CLI 入口。
...
## Phase 17-3：入口、渲染与通知边界

- `generate.py` 从 2065 行收缩为 17 行，只负责把命令行参数交给 `src.pipeline.run()`。
- 模型卡片渲染迁移到 `src/rendering/cards.py`，中英文转换迁移到 `src/rendering/i18n.py`。
- Telegram 刷新摘要与网络发送迁移到 `src/notifications.py`，并支持注入网络边界进行单元测试。
- `src/pipeline.py` 保留跨采集器的顺序编排；采集、定价、发布、监控、通知和渲染的具体职责均由独立模块承担。

迁移期间 `index.html` 与 `en/index.html` 均与 Phase 17-2 基线逐字节一致。关键验证命令必须以失败即停止方式执行，避免恢复步骤覆盖前序失败退出码。
